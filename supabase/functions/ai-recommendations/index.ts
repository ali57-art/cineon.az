...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Sən film və serial mütəxəssisisən. İstifadəçiyə ${language} dilində cavab ver.`;
    const userPrompt = `${genre ? `${genre} janrında` : ""} ${type || "film"} tövsiyə et. 5 uyğun film/serial adı ver və hər biri haqqında 1-2 cümlə ilə qısa təsvir yaz. Cavabı JSON formatında ver: [{"title": "film adı", "description": "qısa təsvir"}]`;
